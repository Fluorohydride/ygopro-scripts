...
function c33846209.costfilter(c,ec,tp)
	...
	return Duel.IsExistingTarget(nil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,aux.GetLeaveFieldGroup(c,ec))
end
function c33846209.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.CheckReleaseGroup(tp,c33846209.costfilter,1,nil,c,tp) end
	local sg=Duel.SelectReleaseGroup(tp,c33846209.costfilter,1,1,nil,c,tp)
...
function c33846209.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		return e:IsCostChecked()
			or Duel.IsExistingTarget(nil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c)
	end
