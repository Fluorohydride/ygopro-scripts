...
function c3734202.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=aux.SelectMatchingCardCrossField(tp,c3734202.spfilter1,tp,LOCATION_DECK,0,1,1,nil,e,tp)
...
function c3734202.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=aux.SelectMatchingCardCrossField(tp,c3734202.spfilter2,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
		Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	end
end
...
function c3734202.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=aux.SelectMatchingCardCrossField(tp,c3734202.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
