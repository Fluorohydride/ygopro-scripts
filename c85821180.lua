...
function c85821180.costfilter(c,e,tp)
	return c:IsType(TYPE_MONSTER) and c:IsDiscardable()
		and Duel.IsExistingMatchingCard(c85821180.filter,tp,LOCATION_DECK,0,1,c,c:GetRace(),c:GetAttribute(),c:GetLevel(),e,tp)
end
...
function c85821180.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(c85821180.costfilter,tp,LOCATION_HAND,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local g=Duel.SelectMatchingCard(tp,c85821180.costfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
