...
function c71039903.initial_effect(c)
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c71039903.spcon)
	e2:SetTarget(c71039903.sptg)
	e2:SetOperation(c71039903.spop)
	c:RegisterEffect(e2)
	--tohand
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(71039903,1))
	e3:SetCategory(CATEGORY_TOHAND)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_GRAVE)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetCountLimit(1,71039903)
	e3:SetCost(c71039903.thcost)
	e3:SetTarget(c71039903.thtg)
	e3:SetOperation(c71039903.thop)
	c:RegisterEffect(e3)
end
function c71039903.regop(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():RegisterFlagEffect(71039903,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
end
...
function c71039903.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c71039903.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c71039903.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c71039903.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
