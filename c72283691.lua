...
function c72283691.spop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToChain(0) or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
...
function c72283691.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToChain(0) and not tc:IsStatus(STATUS_ATTACK_CANCELED) and Duel.Destroy(tc,REASON_EFFECT)~=0 then
