...
function c68250822.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if op==1 then
		e:SetCategory(0)
		Duel.Hint(HINT_OPSELECTED,tp,aux.Stringid(68250822,0))
	...
	elseif op==2 then
		...
		Duel.Hint(HINT_OPSELECTED,tp,aux.Stringid(68250822,1))
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
		local g=Duel.SelectTarget(tp,c68250822.filter2,tp,0,LOCATION_MZONE,1,1,nil,zone)
		Duel.SetOperationInfo(0,CATEGORY_CONTROL,g,1,0,0)
	else
		e:SetCategory(CATEGORY_SPECIAL_SUMMON)
		Duel.Hint(HINT_OPSELECTED,tp,aux.Stringid(68250822,2))
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectTarget(tp,c68250822.filter3,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,e,tp,zone)
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
	end
end
function c68250822.operation(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		c68250822.operation1(e,tp,eg,ep,ev,re,r,rp)
	elseif op==2 then
		c68250822.operation2(e,tp,eg,ep,ev,re,r,rp)
	else
		c68250822.operation3(e,tp,eg,ep,ev,re,r,rp)
	end
end
