...
function c61665245.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummonStep(tc,0,tp,1-tp,false,false,POS_FACEUP_DEFENSE,zone) then
		...
		if Duel.IsExistingMatchingCard(c61665245.spfilter2,tp,LOCATION_DECK,0,1,nil,e,tp,tc:GetRace()) and Duel.SelectYesNo(tp,aux.Stringid(61665245,1)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local sc=Duel.SelectMatchingCard(tp,c61665245.spfilter2,tp,LOCATION_DECK,0,1,1,nil,e,tp,tc:GetRace()):GetFirst()
			if Duel.SpecialSummonStep(sc,0,tp,sump,false,false,POS_FACEUP) then
				local e1=Effect.CreateEffect(c)
				e1:SetType(EFFECT_TYPE_SINGLE)
				e1:SetCode(EFFECT_DISABLE)
				e1:SetReset(RESET_EVENT+RESETS_STANDARD)
