...
function Auxiliary.XyzAlterFilter(c,alterf,xyzc,e,tp,op,sg)
	return alterf(c) and c:IsCanBeXyzMaterial(xyzc) and Duel.GetLocationCountFromEx(tp,tp,c,xyzc)>0 and (not op or op(e,tp,0,c)) and (not sg or #sg<=1 and (#sg==0 or sg:IsContains(c)))
end
--Xyz monster, lv k*n
...
function Auxiliary.XyzLevelFreeTarget(f,gf,minct,maxct)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk,c,og,min,max)
				...
				local g=Auxiliary.SelectGroup(tp,HINTMSG_XMATERIAL,mg,Auxiliary.XyzLevelFreeGoal,sg,minc,maxc,tp,c,gf)
				if g:GetCount()>0 then
					g:KeepAlive()
					e:SetLabelObject(g)
					return true
...
function Auxiliary.XyzLevelFreeCondition2(f,gf,minct,maxct,alterf,desc,op)
	return	function(e,c,og,min,max)
				...
				local altg=mg:Filter(Auxiliary.XyzAlterFilter,nil,alterf,c,e,tp,op,sg)
...
function Auxiliary.XyzLevelFreeTarget2(f,gf,minct,maxct,alterf,desc,op)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk,c,og,min,max)
				...
				local b1=Auxiliary.CheckGroup(ag,Auxiliary.XyzLevelFreeGoal,sg,minc,maxc,tp,c,gf)
				local b2=(not min or min<=1) and mg:IsExists(Auxiliary.XyzAlterFilter,1,nil,alterf,c,e,tp,op,sg)
				if b2 and (not b1 or Duel.SelectYesNo(tp,desc)) then
					e:SetLabel(1)
					if g:GetCount()==0 then
					...
					e:SetLabel(0)
					g=Auxiliary.SelectGroup(tp,HINTMSG_XMATERIAL,ag,Auxiliary.XyzLevelFreeGoal,sg,minc,maxc,tp,c,gf)
				end
				if g:GetCount()>0 then
					g:KeepAlive()
					e:SetLabelObject(g)
...
function Auxiliary.RPECheck(g,tp,c)
	local ct=g:GetCount()
	return g:CheckWithSumEqual(Card.GetRitualLevel,c:GetOriginalLevel(),ct,ct,c) and Duel.GetMZoneCount(tp,g,tp)>0
end
function Auxiliary.RPEFilter(c,filter,e,tp,m)
	...
	return Auxiliary.CheckGroup(mg,Auxiliary.RPECheck,nil,1,99,tp,c)
end
...
function Auxiliary.RPEOperation(filter)
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				if tc then
					...
					local mat=Auxiliary.SelectGroup(tp,HINTMSG_RELEASE,mg,Auxiliary.RPECheck,nil,1,99,tp,tc)
					tc:SetMaterial(mat)
					Duel.ReleaseRitualMaterial(mat)
					Duel.BreakEffect()
					Duel.SpecialSummon(tc,SUMMON_TYPE_RITUAL,tp,tp,false,true,POS_FACEUP)
...
function Auxiliary.RPECheck2(g,tp,c)
	local ct=g:GetCount()
	return g:CheckWithSumEqual(Card.GetRitualLevel,c:GetLevel(),ct,ct,c) and Duel.GetMZoneCount(tp,g,tp)>0
end
function Auxiliary.RPEFilter2(c,filter,e,tp,m)
	...
	return Auxiliary.CheckGroup(mg,Auxiliary.RPECheck2,nil,1,99,tp,c)
end
...
function Auxiliary.RPEOperation2(filter)
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				if tc then
					...
					local mat=Auxiliary.SelectGroup(tp,HINTMSG_RELEASE,mg,Auxiliary.RPECheck2,nil,1,99,tp,tc)
					tc:SetMaterial(mat)
					Duel.ReleaseRitualMaterial(mat)
					Duel.BreakEffect()
					Duel.SpecialSummon(tc,SUMMON_TYPE_RITUAL,tp,tp,false,true,POS_FACEUP)
...
function Auxiliary.LinkCondition(f,minc,maxc,gf)
	return	function(e,c)
				...
				return Auxiliary.CheckGroup(mg,Auxiliary.LCheckGoal,sg,minc,maxc,tp,c,gf)
			end
end
