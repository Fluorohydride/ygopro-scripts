...
--
function Duel.SelectMatchingCard(tp, filter, viewer, field_self, field_oppo, min, max, exception, ...)
	...
	g:Sub(g1)
	g:Sub(g2)
	if can_search_1 and ((g:GetCount()+g2:GetCount())<min
		or (g1:GetCount()>0 and Duel.SelectYesNo(tp,600))
		or (Duel.SelectYesNo(tp,601))) then
		has_searched_1=true
		g:Merge(g1)
	end
	if can_search_2 and (g:GetCount()<min
		or (g2:GetCount()>0 and Duel.SelectYesNo(tp,602))
		or (Duel.SelectYesNo(tp,603))) then
		has_searched_2=true
		g:Merge(g2)
	end
