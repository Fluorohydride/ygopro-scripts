...
function c17760003.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsCode,68505803),aux.NonTuner(c,nil),1)
	c:EnableReviveLimit()
	--mat check
...
function c17760003.regop(e,tp,eg,ep,ev,re,r,rp)
	...
	if bit.band(att,ATTRIBUTE_WIND)~=0 then
		...
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(17760003,3))
	end
	if bit.band(att,ATTRIBUTE_WATER)~=0 then
		...
		e1:SetReset(RESET_EVENT+RESETS_STANDARD)
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(17760003,4))
	end
	if bit.band(att,ATTRIBUTE_DARK)~=0 then
		...
		e1:SetReset(RESET_EVENT+RESETS_STANDARD)
		c:RegisterEffect(e1)
		c:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(17760003,5))
	end
end
