...
function c44362883.filter2(c,e,tp,m,f,chkf)
	...
	aux.FGoalCheckAdditional=c.branded_fusion_check or c44362883.fcheck
	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c44362883.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf,chkf)
			end
		end
		return res
	end
...
function c44362883.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or ce and not Duel.SelectYesNo(tp,ce:GetDescription())) then
			aux.FCheckAdditional=c44362883.fcheck
			aux.FGoalCheckAdditional=tc.branded_fusion_check or c44362883.fcheck
