...
function c31563350.filter(c)
	return c:IsRace(RACE_WARRIOR) and c:IsAbleToEquip()
end
function c31563350.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c31563350.filter,tp,LOCATION_HAND,0,1,nil) end
...
function c31563350.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c31563350.filter,tp,LOCATION_HAND,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
