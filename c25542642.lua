...
function c25542642.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--cannot attack/battle target/disable
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_FIELD)
	e3:SetCode(EFFECT_CANNOT_ATTACK)
	e3:SetRange(LOCATION_SZONE)
	e3:SetTargetRange(LOCATION_MZONE,LOCATION_MZONE)
	e3:SetTarget(c25542642.ctg)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	...
	e6:SetDescription(aux.Stringid(25542642,0))
	e6:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e6:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e6:SetType(EFFECT_TYPE_QUICK_O)
	e6:SetCode(EVENT_FREE_CHAIN)
	e6:SetRange(LOCATION_GRAVE)
	e6:SetCountLimit(1,25542642)
	e6:SetCost(c25542642.spcost)
	e6:SetTarget(c25542642.sptg)
	e6:SetOperation(c25542642.spop)
	c:RegisterEffect(e6)
end
