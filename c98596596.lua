...
function c98596596.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsPlayerCanSpecialSummonMonster(tp,23116809,0,0x4011,100,100,1,RACE_PYRO,ATTRIBUTE_FIRE) then
...
function c98596596.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if not e:GetHandler():IsRelateToEffect(e) or ct<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,23116809,0,0x4011,100,100,1,RACE_PYRO,ATTRIBUTE_FIRE) then return end
	if Duel.IsPlayerAffectedByEffect(tp,59822133) then ct=1 end
	for i=1,ct do
		local token=Duel.CreateToken(tp,98596597)
