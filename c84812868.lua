...
function c84812868.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(84812868,1))
...
function c84812868.thfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
function c84812868.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and c84812868.thfilter(chkc) end
	local ct=Duel.GetMatchingGroupCount(c84812868.filter,tp,LOCATION_ONFIELD,0,e:GetHandler())
	if chk==0 then return ct>0 and Duel.IsExistingTarget(c84812868.thfilter,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c84812868.thfilter,tp,0,LOCATION_ONFIELD,1,ct,nil)
