...
function c53701259.initial_effect(c)
	...
	g:KeepAlive()
	aux.RegisterEachTimeEvent(c,EVENT_SUMMON_SUCCESS,c53701259.cfilter,g)
	aux.RegisterEachTimeEvent(c,EVENT_SPSUMMON_SUCCESS,c53701259.cfilter,g)
...
function c53701259.cfilter(c,e,tp)
	return c:IsSummonPlayer(1-tp) and c:IsFaceup()
		and Duel.IsExistingMatchingCard(c53701259.filter,tp,LOCATION_ONFIELD,0,1,nil)
end
function c53701259.lpcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c53701259.cfilter,1,nil,e,tp) and not Duel.IsChainSolving()
end
function c53701259.lpop1(e,tp,eg,ep,ev,re,r,rp)
	local lg=eg:Filter(c53701259.cfilter,nil,e,tp)
...
function c53701259.lpop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local lg=e:GetLabelObject():Filter(Card.IsLocation,nil,LOCATION_MZONE)
	local rnum=lg:GetSum(Card.GetAttack)
	Duel.Recover(tp,rnum,REASON_EFFECT)
end
