...
function c5795980.tgfilter(c,tc,ec)
	return aux.disfilter1(c,true) and c:GetEquipTarget()~=tc and c~=ec
end
function c5795980.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsOnField() and aux.disfilter1(chkc,true) and chkc~=c end
	if chk==0 then
		...
		if e:GetLabel()==1 then
			...
			return Duel.IsExistingTarget(aux.disfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c,true)
		end
	end
	if e:GetLabel()==1 then
		e:SetLabel(0)
	...
	local g=Duel.SelectTarget(tp,aux.disfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,c,true)
