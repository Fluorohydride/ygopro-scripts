...
function c9175957.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		local ct=Duel.GetMatchingGroupCount(c9175957.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil,e,tp)
		local spct=e:GetLabel()
		if spct>0 and ct>=spct and ft>=spct
			and Duel.SelectYesNo(tp,aux.Stringid(9175957,2)) then
			Duel.BreakEffect()
			local sg=Duel.SelectMatchingCard(tp,c9175957.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,spct,spct,nil,e,tp)
