...
function c16404809.initial_effect(c)
	aux.RegisterSummonFlag(c,EVENT_SUMMON_SUCCESS,16404809)
	--mill
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(16404809,0))
	e1:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND+CATEGORY_DECKDES)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCode(EVENT_PHASE+PHASE_END)
	e1:SetCountLimit(1)
	e1:SetCondition(c16404809.condition)
	e1:SetCost(c16404809.cost)
	e1:SetTarget(c16404809.target)
	e1:SetOperation(c16404809.operation)
	c:RegisterEffect(e1)
end
