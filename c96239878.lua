...
function c96239878.mtfilter1(c)
    return (c:IsCode(89631139) or c:IsCode(46986414))  and c:IsType(TYPE_MONSTER)
end
function c96239878.mtfilter2(c)
    return (c:IsFusionCode(89631139) or c:IsFusionCode(46986414)) and c:IsFusionType(TYPE_MONSTER)
end
function c96239878.rmcon(e,tp,eg,ep,ev,re,r,rp)
    return eg:GetFirst():IsType(e:GetLabel()) and eg:GetFirst():GetFlagEffect(96239878)~=0
end
...
function c96239878.valcheck(e,c)
    ...
    elseif c:IsType(TYPE_FUSION) and g:IsExists(Card.IsFusionCode,1,nil,25451652) g:IsExists(c96239878.mtfilter2,1,nil) then
        c:RegisterFlagEffect(96239878,RESET_EVENT+0x4fe0000+RESET_PHASE+PHASE_END,0,1)
    end
end
...
function c96239878.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c96239878.thfilter(c) end
...
function c96239878.thop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if sc:IsRelateToEffect(e) then
        if sc:IsAbleToDeck() and (not sc:IsAbleToHand() or Duel.SelectOption(tp,1190,1152)==1) then
