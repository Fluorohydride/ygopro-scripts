...
function c44874522.valcheck(e,c)
	...
	local tc=g:GetFirst()
	while tc do
		if tc:IsSetCard(0x7) then and tc:IsType(TYPE_MONSTER) flag=bit.bor(flag,0x1) end
		if tc:IsSetCard(0x51) then and tc:IsType(TYPE_MONSTER) flag=bit.bor(flag,0x2) end
		tc=g:GetNext()
	end
	e:SetLabel(flag)
end
