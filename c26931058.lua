...
function c26931058.filter2(c,ec)
	return c:IsFaceup() and ec:CheckEquipTarget(c) and not c:IsHasEffect(EFFECT_OLD_UNION)
end
...
function c26931058.efop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
		...
		if tc1:IsFaceup() and tc2:IsFaceup() and tc1:IsRelateToEffect(e) and tc2:IsRelateToEffect(e) and not tc2:IsHasEffect(EFFECT_OLD_UNION) and Duel.Equip(tp,tc1,tc2,false) then
