...
function c28781003.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.CheckMustMaterial(tp,c,EFFECT_MUST_BE_XMATERIAL)
...
function c28781003.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFaceup() and c:IsRelateToEffect(e) and c:IsControler(tp) and not c:IsImmuneToEffect(e) and Duel.CheckMustMaterial(tp,c,EFFECT_MUST_BE_XMATERIAL) then
