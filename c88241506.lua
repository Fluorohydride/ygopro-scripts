...
function c88241506.naop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateAttack() and c:IsRelateToEffect(e) then
		...
		local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(c88241506.spfilter,e),tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,nil,e,tp)
...
function c88241506.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c88241506.spfilter,e),tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
