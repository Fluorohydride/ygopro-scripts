...
function c98301564.initial_effect(c)
	...
	e3:SetValue(c98301564.costchange2)
	c:RegisterEffect(e3)
	--search
...
function c98301564.costchange(e,re,rp,val)
	if re and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_TRAP) and re:IsActiveType(TYPE_COUNTER) then
		return 0
	else
		return val
	end
end
function c98301564.costchange2(e,re,r,rp,min,max)
	if re and re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_TRAP) and re:IsActiveType(TYPE_COUNTER) and bit.band(r,REASON_COST)~=0 then
		return 0
	else
		return min,max
	end
