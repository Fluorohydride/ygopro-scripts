...
EFFECT_SYNCHRO_MATERIAL			=232  --受此效果的对方场上的怪兽和场上的魔法&陷阱区域的怪兽卡可用作同调素材
EFFECT_XYZ_MATERIAL				=233	--reserve
EFFECT_FUSION_SUBSTITUTE		=234	--代替指定卡名的融合素材
EFFECT_CANNOT_BE_FUSION_MATERIAL	=235--不能做融合素材
EFFECT_CANNOT_BE_SYNCHRO_MATERIAL	=236--不能做同调素材
EFFECT_SYNCHRO_MATERIAL_CUSTOM		=237--N/A
...
EFFECT_EXTRA_RITUAL_MATERIAL		=243--在墓地·额外卡组等区域可以为仪式召唤所使用
EFFECT_NONTUNER						=244--作为同调素材时可以当作调整以外的怪兽（幻影王 幽骑）
EFFECT_OVERLAY_REMOVE_REPLACE		=245--代替去除超量素材
EFFECT_SCRAP_CHIMERA				=246--N/A
EFFECT_TUNE_MAGICIAN_X				=247--作为超量素材的场合，其他素材必须都符合这个条件（调弦之魔术师）
EFFECT_TUNE_MAGICIAN_F				=248--作为融合素材的场合，其他素材必须都符合这个条件（调弦之魔术师）
...
EFFECT_SYNCHRO_CHECK			=310	--作为同调素材时，可以利用 Card.AssumeProperty 来暂时修改其他同调素材的部分特征(基因组斗士)
...
EFFECT_TUNER_MATERIAL_LIMIT		=353	--作为同调素材的场合，其他同调素材必须符合某一条件
...
EFFECT_HAND_SYNCHRO_MATERIAL	=369	--这张卡作为同调素材的场合，手卡的1只符合条件的怪兽也能作为同调素材
...
GLOBALFLAG_SCRAP_CHIMERA		=0x4		--N/A
