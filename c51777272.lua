...
function c51777272.repfilter(c,tp)
	return c:IsControler(tp) and c:IsOnField() and c:IsReason(REASON_BATTLE) and c:GetFlagEffect(51777272)==0
end
function c51777272.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c51777272.repfilter,1,nil,1-tp) end
	local g=eg:Filter(c51777272.repfilter,nil,1-tp)
	local tc=g:GetFirst()
	while tc do
