...
function c2376209.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetFlagEffect(2376209)==0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,2376209,0xd4,0x11,1200,0,2,RACE_AQUA,ATTRIBUTE_WATER) end
...
function c2376209.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp,2376209,0xd4,0x11,1200,0,2,RACE_AQUA,ATTRIBUTE_WATER) then
		...
		e3:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_OVERLAY-RESET_TOFIELD-RESET_LEAVE)
