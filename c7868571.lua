...
function c7868571.desfilter(c,tp,ft)
...
function c7868571.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and c7868571.desfilter(chkc,ft) and chkc~=e:GetHandler() end
	if chk==0 then return Duel.IsExistingTarget(c7868571.desfilter,tp,LOCATION_ONFIELD,0,1,e:GetHandler(),ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c7868571.desfilter,tp,LOCATION_ONFIELD,0,1,1,e:GetHandler(),ft)
