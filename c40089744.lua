...
function c40089744.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=aux.SelectMatchingCardCrossField(tp,c40089744.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c40089744.thop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=aux.SelectMatchingCardCrossField(tp,c40089744.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
	end
