...
function c78474168.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToChain(0) and tc:IsFaceup() and not tc:IsDisabled() and tc:IsControler(1-tp) then
...
function c78474168.activate2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToChain(0) and tc:IsFaceup() and not tc:IsDisabled() and tc:IsControler(1-tp) and tc:IsType(TYPE_EFFECT) then
		Duel.NegateRelatedChain(tc,RESET_TURN_SET)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_DISABLE)
