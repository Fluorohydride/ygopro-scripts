...
function c84218527.filter(c,e,tp)
	return c:IsSetCard(0x31) and c:IsFaceup() and c:IsCanBeSpecialSummoned(e,SUMMON_VALUE_FORTUNE_LADY,tp,false,false)
end
...
function c84218527.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()<=ft then
		local tc=g:GetFirst()
		while tc do
			Duel.SpecialSummonStep(tc,SUMMON_VALUE_FORTUNE_LADY,tp,tp,false,false,POS_FACEUP)
